...
async function apiRequest(path, options = {}) {
  if (!API_BASE) {
    ...
    if (method === 'POST' && path === '/inventory') {
      ...
      item.stock += amount;
    } else if (method === 'POST' && path === '/services') {
      const name = String(payload.name || '').trim();
      const price = Number(payload.price);
      if (!name || !Number.isFinite(price) || price < 0) throw new Error('Nama dan harga layanan wajib diisi.');
      data.services.push({ id: `service-${Date.now()}`, name, price, unit: payload.unit || 'per item', icon: '✦', impacts: [] });
    } else if (method === 'PATCH' && path.startsWith('/services/')) {
      const id = path.split('/')[2];
      const service = data.services.find((entry) => entry.id === id);
      if (!service) throw new Error('Layanan tidak ditemukan.');
      if (payload.name !== undefined) service.name = String(payload.name).trim();
      if (payload.price !== undefined) service.price = Number(payload.price);
      if (payload.unit) service.unit = String(payload.unit);
    } else if (method === 'DELETE' && path.startsWith('/services/')) {
      const id = path.split('/')[2];
...
function renderServicesAdmin() { $('[data-service-admin]').innerHTML = data.services.map((service) => `<article class="admin-service"><div class="admin-service-head"><span class="service-symbol">${service.icon}</span><span class="count-label">${service.unit}</span></div><h3>${service.name}</h3><p>Bahan otomatis terpakai saat transaksi</p><strong>${money(service.price)}</strong><div class="service-actions"><button class="small-button" data-edit-service="${service.id}">Edit</button><button class="small-button danger" data-delete-service="${service.id}">Hapus</button></div></article>`).join(''); $$('[data-edit-service]').forEach((button) => button.addEventListener('click', () => openServiceModal(button.dataset.editService))); $$('[data-delete-service]').forEach((button) => button.addEventListener('click', () => deleteService(button.dataset.deleteService))); }
async function deleteService(id) { const service = data.services.find((item) => item.id === id); if (!service || !confirm(`Hapus layanan "${service.name}"?`)) return; try { const payload = await apiRequest(`/services/${id}`, { method: 'DELETE' }); data = payload.state; delete cart[id]; renderServicesAdmin(); renderServices(); renderReceipt(); showToast(`${service.name} dihapus.`); } catch (error) { showToast(error.message); } }
